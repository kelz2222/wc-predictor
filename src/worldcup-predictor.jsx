...
const FLAG_MAP = {
  ...
  "Spain":"🇪🇸","Portugal":"🇵🇹","Netherlands":"🇳🇱","USA":"🇺🇸","United States":"🇺🇸",
  "Japan":"🇯🇵","Morocco":"🇲🇦","Senegal":"🇸🇳","Italy":"🇮🇹","Croatia":"🇭🇷",
  "Belgium":"🇧🇪","Denmark":"🇩🇰","Mexico":"🇲🇽","Ecuador":"🇪🇨","Canada":"🇨🇦",
  "Uruguay":"🇺🇾","Colombia":"🇨🇴","Chile":"🇨🇱","Serbia":"🇷🇸","Switzerland":"🇨🇭",
  "Austria":"🇦🇹","Turkey":"🇹🇷","Poland":"🇵🇱","Ukraine":"🇺🇦","Australia":"🇦🇺",
  "South Korea":"🇰🇷","Korea Republic":"🇰🇷","Iran":"🇮🇷","Saudi Arabia":"🇸🇦",
  "Cameroon":"🇨🇲","Ghana":"🇬🇭","Nigeria":"🇳🇬","Egypt":"🇪🇬","Qatar":"🇶🇦",
  "Wales":"🏴󠁧󠁢󠁷󠁬󠁳󠁿","Scotland":"🏴󠁧󠁢󠁳󠁣󠁴󠁿","Costa Rica":"🇨🇷","Panama":"🇵🇦",
  "Venezuela":"🇻🇪","Peru":"🇵🇪","Paraguay":"🇵🇾","Bolivia":"🇧🇴",
  "Algeria":"🇩🇿","Tunisia":"🇹🇳","Mali":"🇲🇱","Ivory Coast":"🇨🇮",
  "DR Congo":"🇨🇩","South Africa":"🇿🇦","Indonesia":"🇮🇩","China":"🇨🇳",
  "Iraq":"🇮🇶","UAE":"🇦🇪","New Zealand":"🇳🇿","Slovakia":"🇸🇰",
  "Czech Republic":"🇨🇿","Hungary":"🇭🇺","Romania":"🇷🇴","Greece":"🇬🇷",
  "Sweden":"🇸🇪","Norway":"🇳🇴","Finland":"🇫🇮","Iceland":"🇮🇸",
};
const getFlag = (name) => FLAG_MAP[name] || "🏳️";

const FALLBACK_MATCHES = [
  { id:1,  group:"A", home:"🇨🇦 Canada",    away:"🇲🇽 Mexico",      date:"Jun 11", time:"20:00", venue:"SoFi Stadium",     status:"SCHEDULED" },
  { id:2,  group:"A", home:"🇺🇸 USA",       away:"🏴󠁧󠁢󠁷󠁬󠁳󠁿 Wales",       date:"Jun 12", time:"17:00", venue:"MetLife Stadium",   status:"SCHEDULED" },
  { id:3,  group:"B", home:"🇦🇷 Argentina", away:"🇲🇦 Morocco",     date:"Jun 12", time:"20:00", venue:"Rose Bowl",          status:"SCHEDULED" },
  { id:4,  group:"B", home:"🇧🇷 Brazil",    away:"🇳🇬 Nigeria",     date:"Jun 13", time:"17:00", venue:"AT&T Stadium",       status:"SCHEDULED" },
  { id:5,  group:"C", home:"🇫🇷 France",    away:"🇩🇰 Denmark",     date:"Jun 13", time:"20:00", venue:"Levis Stadium",      status:"SCHEDULED" },
  { id:6,  group:"C", home:"🇦🇺 Australia", away:"🇸🇳 Senegal",     date:"Jun 14", time:"14:00", venue:"Hard Rock Stadium",  status:"SCHEDULED" },
  { id:7,  group:"D", home:"🇩🇪 Germany",   away:"🇯🇵 Japan",       date:"Jun 14", time:"17:00", venue:"Gillette Stadium",   status:"SCHEDULED" },
  { id:8,  group:"D", home:"🇪🇸 Spain",     away:"🇨🇷 Costa Rica",  date:"Jun 14", time:"20:00", venue:"Lincoln Financial",  status:"SCHEDULED" },
  { id:9,  group:"E", home:"🇵🇹 Portugal",  away:"🇺🇾 Uruguay",     date:"Jun 15", time:"17:00", venue:"SoFi Stadium",       status:"SCHEDULED" },
  { id:10, group:"E", home:"🇧🇪 Belgium",   away:"🇸🇦 Saudi Arabia",date:"Jun 15", time:"20:00", venue:"MetLife Stadium",    status:"SCHEDULED" },
  { id:11, group:"F", home:"🇭🇷 Croatia",   away:"🇨🇦 Canada",      date:"Jun 16", time:"14:00", venue:"Rose Bowl",          status:"SCHEDULED" },
  { id:12, group:"F", home:"🇲🇦 Morocco",   away:"🇨🇴 Colombia",    date:"Jun 16", time:"17:00", venue:"Allegiant Stadium",  status:"SCHEDULED" },
];
...
const CHALLENGES = [
  ...
  { id:5, icon:"📊", title:"Group Guru",      desc:"Predict all Group A matches",     pts:120, goal:6, key:"groupA"  },
  { id:6, icon:"👑", title:"World Champion",  desc:"Top the leaderboard for 3 days",  pts:500, goal:3, key:"topDays" },
];
const PLANS = [
  { id:"free",  label:"Free",  price:0,     features:["12 predictions/round","Public leaderboard","Basic badges"] },
  { id:"pro",   label:"Pro",   price:4.99,  features:["Unlimited predictions","Private leagues","AI match tips","Share card","Priority support"], popular:true },
  { id:"elite", label:"Elite", price:14.99, features:["Everything in Pro","Prize pool entry","Analytics dashboard","Custom avatar","Early access"] },
];
...
export default function App() {
    ...
    const fetchMatches = async () => {
      try {
        ...
        if (API_KEY === "YOUR_API_KEY_HERE") {
          setMatches(FALLBACK_MATCHES); setApiStatus("fallback"); setLoadingMatches(false); return;
        }
        const res = await fetch("https://api.football-data.org/v4/competitions/WC/matches?season=2026", { headers: { "X-Auth-Token": API_KEY } });
        if (!res.ok) throw new Error(`API ${res.status}`);
        const data = await res.json();
        const formatted = (data.matches || []).map(m => ({
          id: m.id,
          group: m.group?.replace("GROUP_","") ?? m.stage?.replace(/_/g," ") ?? "?",
          home: `${getFlag(m.homeTeam.name)} ${m.homeTeam.name}`,
          away: `${getFlag(m.awayTeam.name)} ${m.awayTeam.name}`,
        ...
        if (formatted.length > 0) { setMatches(formatted); setApiStatus("live"); }
        else { setMatches(FALLBACK_MATCHES); setApiStatus("fallback"); }
        const results = {};
        (data.matches || []).forEach(m => {
          if (m.status === "FINISHED" && m.score?.fullTime) results[m.id] = { home: m.score.fullTime.home, away: m.score.fullTime.away };
        });
        setLiveResults(results); setLastUpdated(new Date());
      } catch (err) {
        console.warn("Football API unavailable:", err.message);
        setMatches(FALLBACK_MATCHES); setApiStatus("fallback");
      }
  ...
  const handleInstall = async () => {
    ...
    if (outcome === "accepted") { setIsInstalled(true); setInstallBanner(false); showToast("App installed! Find it on your home screen"); }
  ...
  const handleScoreInput = (matchId, side, val) => {
    ...
    if (homeVal !== "" && awayVal !== "") setPredictions(p => ({ ...p, [matchId]: { home: parseInt(homeVal)||0, away: parseInt(awayVal)||0 } }));
  };
  ...
  const submitPrediction = (matchId) => {
    ...
    const result = liveResults[matchId];
    if (result) {
      const pred = predictions[matchId];
      if (pred.home === result.home && pred.away === result.away) {
        setUserPoints(p => p+10); setPerfectCount(p => p+1); setCorrectCount(p => p+1); setStreak(s => s+1);
        showToast("PERFECT SCORE! +10 pts", "perfect");
        setConfetti(true); setTimeout(() => setConfetti(false), 3000);
        setShareModal({ match: matches.find(m=>m.id===matchId), pred, result });
      } else if ((pred.home>pred.away&&result.home>result.away)||(pred.away>pred.home&&result.away>result.home)||(pred.home===pred.away&&result.home===result.away)) {
        setUserPoints(p => p+5); setCorrectCount(p => p+1); setStreak(s => s+1); showToast("Correct result! +5 pts");
      } else { setStreak(0); showToast("Wrong prediction. Keep going!", "error"); }
    } else { showToast("Locked in! Good luck!", "success"); }
  };
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body:JSON.stringify({ model:"claude-sonnet-4-20250514", max_tokens:180,
          messages:[{role:"user",content:`You are a football pundit. Give a sharp 2-sentence prediction tip for this World Cup match: ${match.home} vs ${match.away}. Include a likely scoreline and the key factor. Be bold and fun. End with one emoji: fire, lightning, or target.`}]
        })
      });
      const data = await res.json();
      setAiTip(t => ({ ...t, [match.id]: data.content?.[0]?.text || "Expect a tight contest — this one is too close to call! ⚡" }));
    } catch { setAiTip(t => ({ ...t, [match.id]: "Both teams are in form — this one could go either way. Watch the midfield battle! ⚡" })); }
  ...
  const createLeague = () => {
    ...
    setLeague(l); save("wc_league",l); showToast(`League created! Code: ${code}`);
  };
  const joinLeague = () => {
    const l = { code:leagueCode, name:`League ${leagueCode}`, members:[user.name,"GoalKing99","TikiTaka"], creator:"GoalKing99" };
    setLeague(l); save("wc_league",l); showToast(`Joined league ${leagueCode}!`);
  };
  ...
  const lbData = [...LEADERBOARD_DATA, { name:user?.name||"You", avatar:user?.avatar||"⭐", points:userPoints, streak, badges:[], correct:correctCount, perfect:perfectCount, isUser:true }].sort((a,b)=>b.points-a.points);
      ...
      <div style={{width:"100%",maxWidth:"400px",position:"relative",zIndex:10}}>
        ...
        <div className="card" style={{padding:"28px"}}>
          <div style={{marginBottom:"20px"}}>
            ...
            <input value={onboardName} onChange={e=>setOnboardName(e.target.value)} onKeyDown={e=>e.key==="Enter"&&completeOnboard()} placeholder="Enter your name..."
          ...
          <button className="btn-primary" style={{width:"100%",padding:"14px",fontSize:"17px"}} onClick={completeOnboard}>START PREDICTING →</button>
    ...
    <div style={{fontFamily:"'DM Sans',sans-serif",background:"#080c14",minHeight:"100vh",color:"#fff",position:"relative"}}>
      ...
      {installBanner && !isInstalled && (
        <div style={{position:"fixed",bottom:0,left:0,right:0,zIndex:300,background:"linear-gradient(135deg,#0d1f12,#0a1a1e)",borderTop:"1px solid rgba(0,200,80,0.3)",padding:"14px 16px",display:"flex",alignItems:"center",gap:"12px",boxShadow:"0 -8px 32px rgba(0,0,0,0.5)",animation:"slideUp 0.4s ease"}}>
          ...
          <div style={{flex:1}}>
            <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"17px",letterSpacing:"1px"}}>INSTALL THE APP</div>
        ...
        <div style={{position:"fixed",bottom:0,left:0,right:0,zIndex:300,background:"linear-gradient(135deg,#0d1420,#0a1230)",borderTop:"1px solid rgba(0,150,255,0.3)",padding:"16px",boxShadow:"0 -8px 32px rgba(0,0,0,0.5)",animation:"slideUp 0.4s ease"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"12px"}}>
            <div>
              ...
              <div style={{fontSize:"12px",color:"rgba(255,255,255,0.45)",marginTop:"2px"}}>Install in 2 taps — no App Store needed</div>
            </div>
          ...
          <div style={{display:"flex",alignItems:"stretch"}}>
            {[{icon:"⬆️",text:"Tap Share in Safari toolbar"},{icon:"➕",text:"Tap Add to Home Screen"},{icon:"✅",text:"Tap Add and done!"}].map((s,i)=>(
        ...
        <div style={{maxWidth:"680px",margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div>
            ...
            <div style={{display:"flex",alignItems:"center",gap:"6px"}}>
              ...
              {apiStatus==="live" && <div style={{fontSize:"9px",background:"rgba(0,200,80,0.15)",color:"#00c850",padding:"1px 7px",borderRadius:"10px",letterSpacing:"1px"}}>● LIVE</div>}
              {apiStatus==="fallback" && <div style={{fontSize:"9px",background:"rgba(255,200,0,0.15)",color:"#ffd700",padding:"1px 7px",borderRadius:"10px",letterSpacing:"1px"}}>PREVIEW</div>}
            </div>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:"8px"}}>
            ...
            {isInstalled && <div style={{fontSize:"11px",background:"rgba(0,200,80,0.1)",border:"1px solid rgba(0,200,80,0.2)",color:"#00c850",padding:"5px 10px",borderRadius:"8px"}}>✅ Installed</div>}
            {plan==="free" && <button className="btn-gold" style={{padding:"6px 14px",fontSize:"12px"}} onClick={()=>setUpgradeModal(true)}>⚡ GO PRO</button>}
      ...
      <div style={{position:"relative",zIndex:10,borderBottom:"1px solid rgba(255,255,255,0.05)"}}>
        <div style={{maxWidth:"680px",margin:"0 auto",padding:"12px 16px",display:"flex",gap:"20px",overflowX:"auto"}}>
          {[{label:"POINTS",value:userPoints,color:"#00c850",icon:"⭐"},{label:"CORRECT",value:correctCount,color:"#00aaff",icon:"✅"},{label:"PERFECT",value:perfectCount,color:"#ffd700",icon:"🎯"},{label:"STREAK",value:streak,color:"#ff6400",icon:"🔥"},{label:"RANK",value:`#${userRank}`,color:"#ff69b4",icon:"🏆"}].map(s=>(
      ...
      <div style={{position:"relative",zIndex:10,maxWidth:"680px",margin:"0 auto",padding:"16px 16px 80px"}}>
        ...
        {tab==="predict" && (
          <div>
            ...
            {apiStatus==="fallback" && (
              <div style={{background:"rgba(255,200,0,0.08)",border:"1px solid rgba(255,200,0,0.2)",borderRadius:"10px",padding:"10px 14px",marginBottom:"14px",fontSize:"12px",color:"rgba(255,200,0,0.8)",display:"flex",gap:"8px",alignItems:"flex-start"}}>
                <span style={{flexShrink:0}}>⏳</span>
                <span>Showing preview fixtures. Add your football-data.org API key to get real WC 2026 matches and live scores.</span>
              </div>
            )}
            {apiStatus==="live" && lastUpdated && (
              <div style={{fontSize:"10px",color:"rgba(0,200,80,0.6)",textAlign:"right",marginBottom:"10px"}}>● Live · Updated {lastUpdated.toLocaleTimeString()}</div>
            )}
            <div style={{background:"rgba(255,255,255,0.06)",borderRadius:"4px",height:"4px",marginBottom:"20px",overflow:"hidden"}}>
              <div style={{height:"100%",width:`${matches.length?(Object.keys(submitted).length/matches.length)*100:0}%`,background:"linear-gradient(90deg,#00c850,#00ff88)",transition:"width 0.5s ease",borderRadius:"4px"}}/>
            </div>
            {loadingMatches ? (
              <div style={{textAlign:"center",padding:"60px 20px",color:"rgba(255,255,255,0.4)"}}>
                <div style={{fontSize:"48px",marginBottom:"12px"}}>⚽</div>
                <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"22px",letterSpacing:"1px"}}>LOADING MATCHES...</div>
                <div style={{fontSize:"12px",marginTop:"6px"}}>Fetching live World Cup data</div>
              </div>
            ) : matches.map(m=>(
              <MatchCard key={m.id} match={m} prediction={predictions[m.id]} submitted={submitted[m.id]}
                result={liveResults[m.id]} inputVals={inputVals} aiTip={aiTip[m.id]} loadingTip={loadingTip[m.id]}
          ...
          <div>
            ...
            <div style={{display:"flex",justifyContent:"center",alignItems:"flex-end",gap:"10px",marginBottom:"24px",padding:"20px 0"}}>
              {[lbData[1],lbData[0],lbData[2]].map((p,i)=>{
                const heights=[90,110,75],medals=["🥈","🥇","🥉"],colors=["#c0c0c0","#ffd700","#cd7f32"];
                return p?(
                  <div key={p.name} style={{textAlign:"center",flex:1}}>
                    ...
                    <div style={{background:p.isUser?"rgba(0,200,80,0.2)":"rgba(255,255,255,0.06)",border:`2px solid ${colors[i]}40`,borderRadius:"12px 12px 0 0",height:`${heights[i]}px`,display:"flex",flexDirection:"column",justifyContent:"center",alignItems:"center",padding:"8px"}}>
                      ...
                      <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"16px",color:colors[i]}}>{p.points}</div>
                    </div>
            ...
            {!league?(
              <div style={{display:"grid",gap:"12px"}}>
                <div className="card" style={{padding:"24px"}}>
                  ...
                  <div style={{fontSize:"13px",color:"rgba(255,255,255,0.45)",marginBottom:"16px"}}>Set it up, share the code, track who is the real genius</div>
                  <input value={leagueName} onChange={e=>setLeagueName(e.target.value)} placeholder="League name (e.g. Work Mates 2026)"
                    style={{width:"100%",background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:"10px",padding:"11px 14px",color:"#fff",fontSize:"14px",outline:"none",boxSizing:"border-box",marginBottom:"12px"}}/>
                ...
                <div className="card" style={{padding:"24px"}}>
                  ...
                  <input value={leagueCode} onChange={e=>setLeagueCode(e.target.value.toUpperCase())} placeholder="Enter 6-digit code"
                    style={{width:"100%",background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:"10px",padding:"11px 14px",color:"#fff",fontSize:"16px",outline:"none",boxSizing:"border-box",letterSpacing:"4px",marginBottom:"12px"}}/>
              ...
              <div>
                <div className="card" style={{padding:"20px",marginBottom:"16px",background:"linear-gradient(135deg,rgba(0,200,80,0.1),rgba(0,100,255,0.05))",border:"1px solid rgba(0,200,80,0.25)"}}>
                  ...
                  <button onClick={()=>{navigator.clipboard?.writeText(league.code);showToast("Code copied!");}}
                    style={{marginTop:"14px",background:"rgba(255,255,255,0.07)",border:"1px solid rgba(255,255,255,0.12)",color:"#fff",padding:"8px 16px",borderRadius:"8px",cursor:"pointer",fontSize:"13px"}}>
                    📋 Copy invite code
                  </button>
                </div>
                ...
                {league.members.map((m,i)=>(
                  <div key={m} className="card" style={{padding:"14px 16px",marginBottom:"8px",display:"flex",alignItems:"center",gap:"12px"}}>
                    ...
                    <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"20px"}}>{m===user?.name?userPoints:[147,132,98][i]??45} pts</div>
                  </div>
                ))}
              </div>
            )}
        ...
        {tab==="rewards" && (
          <div>
            <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"20px",letterSpacing:"1px",marginBottom:"4px"}}>CHALLENGES AND REWARDS</div>
            ...
            {CHALLENGES.map((r,i)=>{
              ...
              const pct=Math.min(100,(progress/r.goal)*100),done=pct>=100;
              return (
            ...
            <div style={{marginTop:"20px",background:"linear-gradient(135deg,rgba(255,215,0,0.1),rgba(255,140,0,0.07))",border:"1px solid rgba(255,215,0,0.25)",borderRadius:"16px",padding:"24px",textAlign:"center"}}>
              ...
              <button className="btn-gold" style={{padding:"11px 28px",fontSize:"14px"}} onClick={()=>{navigator.clipboard?.writeText(window.location.href);showToast("Link copied! Share it!");}}>
                🔗 SHARE AND GROW THE POOL
              </button>
            </div>
          </div>
        )}
        ...
        {tab==="upgrade" && (
          <div>
            <div style={{textAlign:"center",marginBottom:"24px"}}>
              ...
              <div style={{fontSize:"13px",color:"rgba(255,255,255,0.45)"}}>Unlock AI tips, private leagues and prize pool entry</div>
            </div>
            {PLANS.map(p=>(
            ...
            <div style={{marginTop:"28px",borderTop:"1px solid rgba(255,255,255,0.07)",paddingTop:"24px"}}>
              ...
              {isInstalled?(
                <div className="card" style={{padding:"20px",textAlign:"center",border:"1px solid rgba(0,200,80,0.3)",background:"rgba(0,200,80,0.06)"}}>
                  ...
                  <div style={{fontSize:"12px",color:"rgba(255,255,255,0.45)",marginTop:"4px"}}>Open from your home screen anytime</div>
                </div>
              ):installPrompt?(
                <div className="card" style={{padding:"20px"}}>
                  <button className="btn-primary" style={{width:"100%",padding:"13px",fontSize:"16px"}} onClick={handleInstall}>📲 INSTALL NOW — ITS FREE</button>
                </div>
              ):isIos?(
                <div className="card" style={{padding:"20px"}}>
                  <div style={{fontWeight:600,fontSize:"15px",marginBottom:"12px"}}>Install on iPhone or iPad</div>
                  {[{n:"1",icon:"⬆️",t:"Tap the Share button",s:"in Safari's bottom toolbar"},{n:"2",icon:"➕",t:"Tap Add to Home Screen",s:"scroll down to find it"},{n:"3",icon:"✅",t:"Tap Add",s:"open from your home screen"}].map(s=>(
                    <div key={s.n} style={{display:"flex",gap:"12px",alignItems:"center",marginBottom:"10px"}}>
                      ...
                      <div><div style={{fontSize:"13px",fontWeight:600}}>{s.t}</div><div style={{fontSize:"11px",color:"rgba(255,255,255,0.4)"}}>{s.s}</div></div>
                    </div>
                  ))}
                </div>
              ):(
        ...
        <Modal onClose={()=>setShareModal(null)}>
          <div style={{textAlign:"center",padding:"8px"}}>
            ...
            <div style={{display:"grid",gap:"10px"}}>
              <button className="btn-primary" style={{padding:"12px"}} onClick={()=>{navigator.clipboard?.writeText(`I just predicted the perfect score! ${shareModal.match?.home} ${shareModal.pred?.home}-${shareModal.pred?.away} ${shareModal.match?.away} Play at ${window.location.href}`);showToast("Share text copied!");}}>
                📋 Copy share text
              </button>
      ...
      {upgradeModal&&(
        <Modal onClose={()=>setUpgradeModal(false)}>
          <div style={{textAlign:"center"}}>
            ...
            <div style={{fontSize:"13px",color:"rgba(255,255,255,0.5)",marginBottom:"20px"}}>AI match tips are a Pro and Elite feature. Upgrade to unlock expert predictions powered by Claude AI.</div>
...
function MatchCard({match,prediction,submitted,result,inputVals,aiTip,loadingTip,onScoreInput,onSubmit,onAiTip,plan}) {
  ...
  const correct=submitted&&result&&prediction&&!perfect&&((prediction.home>prediction.away&&result.home>result.away)||(prediction.away>prediction.home&&result.away>result.home)||(prediction.home===prediction.away&&result.home===result.away));
  const wrong=submitted&&result&&!perfect&&!correct;
  const isLive=match.status==="IN_PLAY"||match.status==="PAUSED";
  return (
    <div className="card" style={{padding:"18px",marginBottom:"12px",border:perfect?"1px solid rgba(0,200,80,0.5)":correct?"1px solid rgba(0,168,255,0.3)":wrong?"1px solid rgba(255,60,60,0.2)":isLive?"1px solid rgba(255,100,0,0.4)":undefined,transition:"all 0.3s"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"12px"}}>
        <div style={{display:"flex",gap:"6px",alignItems:"center"}}>
          ...
          {isLive&&<span style={{fontSize:"10px",background:"rgba(255,100,0,0.2)",color:"#ff6400",padding:"3px 9px",borderRadius:"20px",letterSpacing:"1px",animation:"pulse 1s infinite"}}>● LIVE</span>}
          {match.status==="FINISHED"&&<span style={{fontSize:"10px",background:"rgba(255,255,255,0.08)",color:"rgba(255,255,255,0.4)",padding:"3px 9px",borderRadius:"20px"}}>FT</span>}
        </div>
        <span style={{fontSize:"10px",color:"rgba(255,255,255,0.3)"}}>🕐 {match.date} {match.time}</span>
      </div>
      ...
      {aiTip&&(
        <div style={{background:"rgba(0,120,255,0.1)",border:"1px solid rgba(0,120,255,0.2)",borderRadius:"10px",padding:"10px 12px",marginBottom:"10px",fontSize:"12px",color:"rgba(255,255,255,0.7)",fontStyle:"italic",lineHeight:1.5}}>
          <span style={{color:"#00aaff",fontStyle:"normal",fontWeight:700,fontSize:"10px",letterSpacing:"1px"}}>⚡ AI TIP  </span>{aiTip}
        </div>
      )}
...
function Toast({toast}) {
  ...
  return <div style={{position:"fixed",top:"16px",left:"50%",transform:"translateX(-50%)",zIndex:2000,background:colors[toast.type]||colors.success,color:toast.type==="perfect"?"#000":"#fff",padding:"11px 22px",borderRadius:"12px",fontWeight:700,fontSize:"14px",animation:"slideIn 0.3s ease",boxShadow:"0 8px 32px rgba(0,0,0,0.4)",whiteSpace:"nowrap",maxWidth:"90vw"}}>{toast.msg}</div>;
}
